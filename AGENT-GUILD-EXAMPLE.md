...
Validation passed 22 tests and 30 subtests against an installed wheel built from this contribution, with all declared base dependencies. The tests use normal public imports, construct a real `Task`, and exercise native registration, schema and execution in both modes with mocked HTTP. Network, configuration access and model calls were blocked during the tests. A model-backed agent, optional extras, Docker smoke tests and the full project suite were not exercised. The retained live-response fixture came from an owned service probe and is not evidence of independent adoption.
